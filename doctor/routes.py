...
from doctor import app,db, bcrypt, mail
from flask_login import login_user, current_user, logout_user, login_required
from doctor.models import *
from doctor.forms import RegistrationForm ,Resetrequest, DoctorRegistrationForm,Changepassword, LoginForm, Dquestions, Galleryform, Uprofileform, Dprofileform
...
@app.route('/doctorappoinment/<int:id>',methods=['GET','POST'])
def doctorappoinment(id):
    user = User.query.get_or_404(id)
    userid=user.id
    doctor = Doctor.query.filter_by(owner=userid).all()
    if request.method == 'POST':
        name = request.form ['name']
        ...
        appoinments = DoctorAppoinment(owner=current_user.id ,hosid=userid,name=name,doctor=doctor,reason=reason,contactno=contactno,date = date,status = '')
...
def doctorappoinmentview():
    tasks= DoctorAppoinment.query.filter_by(owner=current_user.id).all()
...
def dappointments():
    tasks= DoctorAppoinment.query.filter_by(hosid=current_user.id).all()
...
def dcontact():
    if request.method == 'POST':
        subject = request.form['subject']
        message = request.form['message']
        new = Contact(usertype = 'hospital',name = current_user.username,email=current_user.email,phone='',subject=subject,message=message)
...
def doctorfeedback():
    tasks = Contact.query.filter_by(usertype='hospital').all()
...
def registerhospital():
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        new = User(username= form.username.data,email=form.email.data, password=hashed_password, usertype= 'hospital',status='null')
        db.session.add(new)
        db.session.commit()
        flash('Your account has been created! You are now able to log in', 'success')
        return redirect('/login')
    return render_template('registerhospital.html',title='Register', form=form)


@app.route('/ahospital')
def ahospital():
    tasks=User.query.filter_by(usertype='hospital',status='null').all()
    return render_template('ahospital.html',tasks=tasks)

@app.route('/ahospitalapprove')
def ahospitalapprove():
    tasks=User.query.filter_by(usertype='hospital',status='approve').all()
    return render_template('ahospitalapprove.html',tasks=tasks)

@app.route('/ahospitalreject')
def ahospitalreject():
    tasks=User.query.filter_by(usertype='hospital',status='reject').all()
    return render_template('ahospitalreject.html',tasks=tasks)

@app.route('/aapprove/<int:id>')
def aapprove(id):
    user= User.query.get_or_404(id)
    user.status='approve'
    email=user.email
...
@app.route('/hadddoctor',methods=['GET','POST'])
def hadddoctor():
    if request.method == 'POST':
        name = request.form['name']
        email = request.form['email']
        ...
        doctor = Doctor(owner = current_user.id,name=name,email=email,phone=phone,specilisation=speci)

        try:
            db.session.add(doctor)
            db.session.commit()
            return redirect('/hdoctorview')
        except:
            return 'There was an issue adding your task'

    else:
        return render_template('hadddoctor.html')

@app.route('/hdoctorview')
def hdoctorview():
    tasks = Doctor.query.filter_by(owner=current_user.id).all()
    return render_template('hdoctorview.html',tasks=tasks)

@app.route('/ddoctor')
def ddoctor():
    return render_template("ddoctor.html")

@app.route('/hdoctordelete/<int:id>')
def hdoctordelete(id):
    delete = Doctor.query.get_or_404(id)
...
@app.route('/uchoosehospital')
def uchoosehospital():
    hos = User.query.filter_by(usertype='hospital',status='approve')
    return render_template('uchoosehospital.html',hos=hos)
